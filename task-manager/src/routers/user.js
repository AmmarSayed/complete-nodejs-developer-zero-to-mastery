...
router.post("/users/logout", auth, async (req, res) => {
  try {
    req.user.tokens = req.user.tokens.filter(({ token }) => token !== req.usedToken);
...
router.delete("/users/me", auth, async (req, res) => {
  try {
    ...
    await sendGoodbyEmail(req.user.email, req.user.name);
    res.send(req.user);
  } catch (error) {
...
const upload = multer({
  limits: {
    fileSize: 1000000, // 1 Mega bite
  },
  fileFilter(req, file, cb) {
    if (!file.originalname.match(/\.(jpg|jpeg|png)$/))
      return cb(new Error("💥 Please upload an image. Allowed Image extentions (jpg|jpeg|png) 💥"));
...
router.post(
  ...
  async (req, res) => {
    // we can access this buffer only if we don't specify the "dest" in multer
